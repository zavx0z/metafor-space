  ...
  constructor({strings, ["_$htmlType$"]: type}, options = {}) {
    ...
    if (DEV_MODE) {
      ...
      if (attrNames.length !== attrNameIndex) {
        throw new Error(`Обнаружены повторяющиеся привязки атрибутов. Это происходит, если в вашем шаблоне есть повторяющиеся атрибуты в теге элемента. Например, "<input ?disabled=\${true} ?disabled=\${false}>" содержит повторяющийся атрибут "disabled". Ошибка обнаружена в следующем шаблоне: \n  strings.join("\${...}")`)
      }
    }
  }
...
export class ChildPart {
  ...
  /** @param {UncompiledTemplateResult} result */
  ...
  _commitIterable(value) {
    ...
    for (const item of value) {
      if (partIndex === itemParts.length) {
        ...
        // TODO: проверить производительность, влияние создания всегда двух частей
