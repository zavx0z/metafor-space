...
/** # Переходы
 ...
 После окончания действия (или в его процессе, если запускается цепочка самопереходов) система проверяет: какие условия перехода удовлетворены при текущем контексте.
 Если условия перехода удовлетворены, совершается переход в новое состояние (возможно, с новым Action).
 Переходы будут заблокированы, пока не прекратится цепочка переходов (то есть пока не окажется, что никакие условия перехода больше не удовлетворены). Лишь тогда система будет разблокирована.
...
/** # Целевые состояния в переходе
 ...
 Описывает возможные целевые состояния в виде объекта, где ключи - это состояния, а значения - условия для перехода.
 @template S - Тип состояния
 @template C - Тип данных контекста
 */
