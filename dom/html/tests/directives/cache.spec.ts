...
describe("cache directive", () => {
    ...
    test("caches templates", () => {
        const renderCached = (condition: any, v: string) => render(html`${cache(condition ? html`
            <div v=${v}></div>` : html`<span v=${v}></span>`)}`, container) // prettier-ignore
    ...
    test("caches compiled templates", () => {
        ...
        const renderCached = (condition: any, v: string) =>
            render(
                html` ${cache(condition ? {_$htmlType$: _$lit_template_1, values: [v]} : {
                    _$htmlType$: _$lit_template_2,
                    values: [v]
                })} `,
                container
            ) // prettier-ignore
    ...
    test("caches templates when switching against non-TemplateResults", () => {
        const renderCached = (condition: any, v: string) =>
            render(html` ${cache(condition ? html`
                <div v=${v}></div> ` : v)} `, container) // prettier-ignore
    ...
    test("caches templates when switching against TemplateResult and undefined values", () => {
        const renderCached = (v: unknown) => render(html`
            <div>${cache(v)}</div> `, container) // prettier-ignore
    ...
    test("cache can be dynamic", () => {
        const renderMaybeCached = (condition: any, v: string) => render(html` ${condition ? cache(html`
            <div v=${v}></div> `) : v} `, container) // prettier-ignore
    ...
    test("async directives disconnect/reconnect when moved in/out of cache", () => {
        ...
        const renderCached = (log: string[], condition: boolean) =>
            render(html`
                <div> ${cache(condition ? html`
                    <div>${disconnectable(log, "a")}</div> ` : html` <span>${disconnectable(log, "b")}</span> `)}
                </div> `, container) // prettier-ignore
        const log: string[] = []
