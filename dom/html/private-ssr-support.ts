 ...
 *
 * В настоящее время мы не создаем минифицированную сборку кода @metafor/html-ssr. Чтобы
 ...
 * к приватным полям этих членов), а затем повторно экспортируем их для использования в @metafor/html-ssr.
 * Это позволяет @metafor/html-ssr оставаться независимым от того, используется ли клиентский код
...
export const _$LH = {
  ...
  patchDirectiveResolve: (
    ...
    if (directiveClass.prototype._$resolve !== resolveOverrideFn) {
      ...
      // путь выполнения. Это возможно, так как @pkg-labs/ssr содержит свой собственный @metafor/html
