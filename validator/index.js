...
export function validateTransitions({tag, transitions, contextDefinition}) {
    if (!Array.isArray(transitions)) {
        sendError({id: tag, message: `Transitions должен быть массивом, получено: ${typeof transitions}`, src: "transitions"})
        return
    }
    ...
    transitions.forEach((transition, index) => {
        if (!transition.from) {
            sendError({id: tag, message: `Отсутствует обязательное поле 'from' в transitions[${index}]`, src: "transitions"})
        }
        ...
        if (!transition.to) {
            ...
            transition.to.forEach((to, toIndex) => {
                if (!to.state) {
                    sendError({id: tag, message: `Отсутствует обязательное поле 'state' в transitions[${index}].to[${toIndex}]`, src: "transitions"})
                }
                if (!to.trigger) {
                    sendError({id: tag, message: `Отсутствует обязательное поле 'trigger' в transitions[${index}].to[${toIndex}]`, src: "transitions"})
                }
            })
        }
    })
 ...
 * @param {import('../types/core.d.ts').CoreDefinition<any, any>} params.core Конфигурация ядра
 ...
 * @param {Object} params Параметры валидации
 * @param {string} params.tag Идентификатор частицы
 * @param {import('../types/create.d.ts').CreateOptions<any, any, any>} params.options Конфигурация инстанса частицы
